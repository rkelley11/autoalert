...
// Share Search
def shareSearchClicked = CustomKeywords.'search.Panel.shareSearch'(search['element'])

CustomKeywords.'test.Assistant.verifyTrue'(shareSearchClicked, 'Share Search was successfully clicked', AfterFailure.STOP)

// Verify test object is visible and text is as expected
testObjectUnderTest = findTestObject('Search Page/Modals/Search - Modals - Title')

CustomKeywords.'test.Assistant.waitForElementVisible'(testObjectUnderTest, 10, AfterFailure.STOP)

CustomKeywords.'test.Assistant.verifyText'(testObjectUnderTest, 'Share Search', 'title', AfterFailure.CONTINUE)

// Verify test object is visible and text is as expected
testObjectUnderTest = findTestObject('Search Page/Modals/Search - Modals - Message')

CustomKeywords.'test.Assistant.waitForElementVisible'(testObjectUnderTest, 10, AfterFailure.STOP)

def expectedMessage = 'Search name matches an existing search and cannot be shared. Save the search under a different name and try again.'

CustomKeywords.'test.Assistant.verifyText'(testObjectUnderTest, expectedMessage, 'message', AfterFailure.CONTINUE)
...
CustomKeywords.'test.Assistant.verifyText'(testObjectUnderTest, 'OK', 'button', AfterFailure.CONTINUE)
