...
// Opening Tab
def tabOpened = CustomKeywords.'dealsheet.bottom.Common.openTab'(BottomSectionTabs2.CHANGE_LOG)

WebUI.verifyEqual(tabOpened, true, FailureHandling.CONTINUE_ON_FAILURE)

// Verify Heading
def mainHeading = findTestObject('Dealsheet/Tabs/Change Log/Dealsheet - Tabs - Change Log - Heading')
...
if (elementVisible) {
    ...
    WebUI.verifyEqual(elementText, 'Change Log', FailureHandling.CONTINUE_ON_FAILURE)
}

// Verify Rows in Grid
def tableBody = findTestObject('Dealsheet/Tabs/Change Log/Dealsheet - Tabs - Change Log - Table Body')

elementVisible = WebUI.waitForElementVisible(tableBody, 10)

WebUI.verifyEqual(elementVisible, true, FailureHandling.CONTINUE_ON_FAILURE)

if (elementVisible) {
	def rows = CustomKeywords.'general.Tables.returnRows'(tableBody)
	
    WebUI.verifyEqual(rows.size() > 0, true, FailureHandling.CONTINUE_ON_FAILURE)
}